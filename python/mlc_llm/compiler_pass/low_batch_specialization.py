...
from .opencl_matmul import OpenCLMatmul, get_low_batch_gemv_config, get_gemm_a4_config, get_gemm_a8_config
...
@tvm.transform.module_pass(opt_level=0, name="LowBatchGemvOpenCLSpecialize")
class LowBatchGemvOpenCLSpecialize:  # pylint: disable=too-few-public-methods
    """A compiler pass that dispatch low-batch-gemm to gemv schedule in opencl."""
    ...
    def transform_module(
        ...
        for g_var, func in mod.functions_items():
            if isinstance(func, tir.PrimFunc):
                low_batch_range = [1, 7]
                sch_rules = [
                    OpenCLMatmul(get_low_batch_gemv_config()),
                    OpenCLMatmul(get_gemm_a4_config())
                ]
                low_batch_funcs = []
                for rule in sch_rules:
                    low_batch_mod = IRModule({})
                    low_batch_mod["main"] = func
                    low_batch_mod = dl.ApplyDefaultSchedule(rule)(low_batch_mod)
                    low_batch_funcs.append(low_batch_mod["main"])
                if any(
                    tvm.ir.structural_equal(low_batch_func, func)
                    for low_batch_func in low_batch_funcs
                ...
                gemm_mod["main"] = func
                gemm_mod = dl.ApplyDefaultSchedule(
                    OpenCLMatmul(get_gemm_a8_config()),
                )(gemm_mod)
                gemm_func = gemm_mod["main"]
                sym_var = list(symbolic_vars)[0]
                body = gemm_func.body
