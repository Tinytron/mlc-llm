...
class CauchyAttention(nn.Module):
    def __init__(self, config: CauchyConfig):
        ...
        self.use_qk_bias = config.qkv_bias

        self.wqkv_pack = nn.Linear(
            in_features=self.hidden_size,
            out_features=(self.num_heads + 2 * self.num_key_value_heads) * self.head_dim,
            bias=self.use_qk_bias,
        )
        self.o_proj = nn.Linear(self.num_heads * self.head_dim, self.hidden_size, bias=False)
        if self.use_qk_norm:
            self.q_norm = nn.RMSNorm(self.head_dim, -1, config.rms_norm_eps, bias=False)
            self.k_norm = nn.RMSNorm(self.head_dim, -1, config.rms_norm_eps, bias=False)
    ...
    def forward(self, hidden_states: Tensor, paged_kv_cache: PagedKVCache, layer_id: int):
        ...
        # 在这里加qk norm的逻辑
        q_, k_, v_ = op.split(qkv, [h_q, h_q + h_kv], axis=2)
        q_ = self.q_norm(q_)
        k_ = self.k_norm(k_)
        qkv = op.concat([q_, k_, v_], dim=2, name="concat")
...
class CauchyEmbedding(nn.Embedding):
...
class CauchyDecoderLayer(nn.Module):
    def __init__(self, config: CauchyConfig):
        ...
        self.self_attn = CauchyAttention(config)
        self.num_experts = config.num_experts
        if self.num_experts == 0:
            self.mlp = CauchyMLP(config)
        else:
            # self.mlp = MiniCPMMoE(config)
            raise NotImplementedError("MoE is not supported in Cauchy model yet.")
...
class CauchyModel(nn.Module):
    def __init__(self, config: CauchyConfig):
        ...
        self.embed_tokens = CauchyEmbedding(config.vocab_size, config.hidden_size)
        self.layers = nn.ModuleList(
            [CauchyDecoderLayer(config) for _ in range(config.num_hidden_layers)]
        )
...
class CauchyForCausalLM(nn.Module):  # pylint: disable=too-many-instance-attributes
    def __init__(self, config: CauchyConfig):
